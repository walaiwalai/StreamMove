...
@Slf4j
@Component
public class AliPanUploader extends Uploader {
    @Resource
    ...
    public void setUp() {
        // 检查一下文件
        boolean isExisted = netDiskCopyService.checkBasePathExist(UploadPlatformEnum.ALI_PAN);
        if (!isExisted) {
            throw new StreamerRecordException(ErrorEnum.INVALID_PARAM);
        }
        log.info("ali pan uploader init success");
    }
    ...
    public boolean upload(String recordPath) throws Exception {
        ...
        for (File targetFile : files) {
            ...
            if (remoteSeverVideo != null) {
                log.info("video has been uploaded to ali pan, file: {}", targetFile.getAbsolutePath());
                continue;
            }
    ...
    private RemoteSeverVideo uploadFile(File targetFile) {
        // 1. 发起网盘copy请求
        String taskId = netDiskCopyService.copyFileToNetDisk(UploadPlatformEnum.ALI_PAN, targetFile);
