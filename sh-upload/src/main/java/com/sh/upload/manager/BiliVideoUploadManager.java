...
public class BiliVideoUploadManager {
    ...
    private List<LocalVideo> fetchLocalVideos(String dirName, BiliVideoUploadTask uploadModel) {
        ...
        List<LocalVideo> localVideoParts = Lists.newArrayList();
        for (String subFileName : subFileNames) {
            ...
            if (uploadModel.getIsUploadFail()) {
                ...
                if (isReUploadFailVideo) {
                    ...
                    localVideoParts.add(LocalVideo.builder()
                            .isFailed(true)
     ...
     * @return
     */
    private List<RemoteSeverVideo> uploadVideoParts(List<LocalVideo> localVideoParts,
            BiliVideoUploadTask uploadModel) {
        log.info("uploadVideoPart Start ...");
        List<RemoteSeverVideo> remoteVideos = Lists.newArrayList();
        for (int i = 0; i < localVideoParts.size(); i++) {
            LocalVideo localVideoPart = localVideoParts.get(i);
            try {
                ...
                log.error("upload video part fail, localVideoPart: {}", localVideoPart.getLocalFileFullPath());
     ...
     */
    private void syncStatus(BiliVideoUploadTask uploadModel, LocalVideo localVideoPart,
            RemoteSeverVideo remoteVideoPart) {
        FileStatusModel fileStatusModel = FileStatusModel.loadFromFile(uploadModel.getDirName());
        UploadVideoPair videoParts = fileStatusModel.getVideoParts();
        ...
        boolean isAlreadyInStatus = exsitedSuccessUploadVideoParts.stream().anyMatch(
                succeedUploadVideo -> StringUtils.equals(succeedUploadVideo.getLocalFileFullPath(),
                        localVideoPart.getLocalFileFullPath()));

        if (isAlreadyInStatus) {
            log.info("Found Exist Video {}", localVideoPart.getTitle());
            return;
        }

        SucceedUploadSeverVideo newSucceedPart = new SucceedUploadSeverVideo();
        newSucceedPart.setLocalFileFullPath(localVideoPart.getLocalFileFullPath());
     ...
     * @return
     */
    public UploadVideoPartResult getPreUploadData(BiliVideoUploadTask uploadModel) {
    ...
    private void postVideos(List<RemoteSeverVideo> remoteVideoParts) {
    ...
    public List<FailUploadVideoChunk> uploadVideoChunks(String dirName, File videoFile) throws Exception {
        ...
        extension.put("upos_uri", biliPreUploadInfo.getUposUri());
    ...
    private Map<String, String> buildUploadChunkHeader(String biliCookies, BiliPreUploadInfoModel biliPreUploadInfo) {
        ...
        uploadChunkHeaders.put("X-Upos-Auth", biliPreUploadInfo.getUposUri());
